...
async function generateArticleText(topic: AutoTopic): Promise<string> {
  console.log(`\n${colors.cyan}📝 Generating article text with OpenAI...${colors.reset}`);
  ...
  try {
    ...
    console.log(`${colors.green}✅ Article text generated successfully${colors.reset}`);
    return content;

  } catch (error: any) {
    console.error(`${colors.red}❌ Failed to generate article text:${colors.reset}`, error.message);
    throw error;
  }
}
...
async function autoGeneratePost() {
  console.log(`\n${colors.bright}${colors.magenta}🤖 AUTO-BLOG GENERATOR FOR PISICOPEDIA.RO${colors.reset}`);
  console.log('='.repeat(60));

  // Check required API keys
  if (!process.env.OPENAI_API_KEY) {
    console.error(`${colors.red}❌ Error: OPENAI_API_KEY is not set in .env.local${colors.reset}`);
    process.exit(1);
  }

  if (!process.env.LEONARDO_API_KEY) {
    console.error(`${colors.red}❌ Error: LEONARDO_API_KEY is not set in .env.local${colors.reset}`);
    process.exit(1);
  }

  // Step 1: Get next topic
  console.log(`\n${colors.cyan}1️⃣ Selecting topic...${colors.reset}`);
  ...
  if (!topic) {
    console.log(`${colors.yellow}⚠️  No new topics available for generation${colors.reset}`);
    console.log('   All topics have been generated or no topics defined');
    process.exit(0);
  }

  console.log(`${colors.green}✅ Selected topic:${colors.reset} ${topic.title}`);
  ...
  try {
    ...
    console.log(`\n${colors.cyan}2️⃣ Generating article text...${colors.reset}`);
    const articleContent = await generateArticleText(topic);

    // Validate generated content
    console.log(`\n${colors.cyan}3️⃣ Validating generated content...${colors.reset}`);
    const validation = validateAutoGeneratedContent(articleContent);

    if (!validation.valid) {
      console.error(`${colors.red}❌ Content validation failed:${colors.reset}`);
    ...
    console.log(`\n${colors.cyan}4️⃣ Generating article image with Leonardo...${colors.reset}`);
    ...
    console.log(`${colors.green}✅ Article saved:${colors.reset} ${filePath}`);

    // Success summary
    console.log('\n' + '='.repeat(60));
    console.log(`${colors.bright}${colors.green}🎉 AUTO-GENERATION COMPLETE!${colors.reset}`);
    console.log('='.repeat(60));
    console.log(`📝 Article: ${topic.title}`);
    console.log(`📁 File: content/articles/${slug}.mdx`);
    ...
    console.error(`\n${colors.red}❌ Auto-generation failed:${colors.reset}`, error.message);
    console.error('\nDebug info:', error);
    process.exit(1);
  }
}
